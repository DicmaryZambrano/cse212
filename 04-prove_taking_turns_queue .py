...
# Defect(s) Found: Enqueue Method added the players on the wrong order (Sue(3),Tim(5),Bob(2)), insteand of inserting them
# all on index 0 append should be used so it creates a queue order (Bob(2),Tim(5),Sue(3)). 
...
for i in range(10):
    players.get_next_person()
#print(players)    
# Defect(s) Found: Program does not check for the infinite turns case, once it decues a person with a turn value of 0
# it never enques them again, i fixed the problem by adding a check for turn values that are equeal to 0
# now the program knows that it has to add the person to the queue an infinite ammount of times if their turn value is 0
...
# overall the program only had a few issues, The queue order did not work properly because it inserted all players into index[0],
# this could be fixed simply by using append to always add players to the end of the queue, the program had no problem
# adding new players even after dequeueing some of them, the program did not check for players that had an infinite turn number
# but this also could be easily fix by addint an if statement that checked for players that ran out of turns, and it had 
# no problem displaying the error message at the end.